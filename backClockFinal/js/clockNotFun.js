var Clock = (function(){
    ...
    var moveClock = function(){
        interval = setTimeout(
            function moveHandClock(){
                countSeconds++;
                rotateHandClock(seconds,countSeconds);
                moveMinutes();
                showDigitaTime();
            interval = setTimeout(moveHandClock,1);
            },1);
    }
    ...
    var moveMinutes = function(){
        if(countSeconds % 60 === 0){
            countSeconds = 0;
            countMinutes++;
            rotateHandClock(minutes,countMinutes);
            moveHours();
        }
    }

    var moveHours = function(){
        if(countMinutes % 12 === 0){
            countHours++;
            if(countHours % 5 === 0){
                countMinutes = 0;
            }
            rotateHandClock(hours,countHours);
            if(countHours === 720){
                countHours = 0;
            }
        }
    ...
    var restartClock = function(h,min,sec){
        countSeconds = sec;
        countMinutes = min;
        var grades = countMinutes % 12;
        countHours = parseInt((h * 30) + grades);
        rotateHandClock(seconds,sec);
        rotateHandClock(minutes,min);
        rotateHandHourClock(hours,countHours);
        grades = countMinutes / 12;
        countHours = h * 5 + grades;
        pauseClock();
    }
    ...
    var setTimeClock = function(){
        ...
        restartClock(h,min,sec);
    }

    var handleEvents = function(){
        time = 0;
        document.getElementById("btnRestart").addEventListener("click", function(){restartClock(time,time,time)});
        ...
        document.getElementById("btnSetTime").addEventListener("click", setTimeClock);
    }

    var showDigitaTime = function(){
        document.getElementById("timer").value = Math.floor(countHours/5) + ":" + countMinutes + ":" + countSeconds;
    }
...
function createDOMClock(){
    ...
    createWrapperContent(divWrapper);
}

function createWrapperContent(divWrapper){
...
function createDivClockContainerContent(divClockContainer){
    ...
    createSvgContent(svgClock);
}

function createDivHoursContent(divHoursContainer){
...
function createDivMinContent(divMinutesContainer){
...
function createDivSecContent(divSecondsContainer){
...
function createSvgContent(svgClock){
    
    var pathClock = document.createElementNS('http://www.w3.org/2000/svg','path');
    ...
    var gClock = document.createElement('g');
    svgClock.appendChild(gClock);

    var pathClock2 = document.createElementNS('http://www.w3.org/2000/svg','path');
...
function createButtons(divButtons){

    var aRestart = document.createElement('a');
    aRestart.setAttribute('id','btnRestart');
    ...
    aPause.setAttribute('id','btnPause');
    ...
    aResume.setAttribute('id','btnPabtnResumeuse');
    aResume.setAttribute('class','actionButton');
    aResume.textContent = "Resume";
    divButtons.appendChild(aResume);

    var aSetTime = document.createElement('a');
    aSetTime.setAttribute('id','btnSetTime');
