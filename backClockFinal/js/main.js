...
var firstCreated = false;
    ...
    var moveClock = function(){
        this.seconds = this.container[this.numCont].children[2].children[0];
        this.minutes = this.container[this.numCont].children[1].children[0];
        this.hours = this.container[this.numCont].children[0].children[0];
        this.btnResume.disabled = true;
        var itSelf = this;
        interval = setTimeout(
            function moveHandClock(){
                itSelf.countSeconds++;
                rotateHandClock.call(itSelf,itSelf.seconds,itSelf.countSeconds);
                moveMinutes.call(itSelf);
                showDigitaTime.call(itSelf);
                interval = setTimeout(moveHandClock.bind(itSelf),10);
                itSelf.interval = interval;
            },10);
    }

    var moveMinutes = function(){
        if(this.countSeconds % 60 === 0){
            this.countSeconds = 0;
            this.countMinutes++;
            rotateHandClock.call(this,this.minutes,this.countMinutes);
            moveHours.call(this);
        }
    }

    var moveHours = function(){
        if(this.countMinutes % 12 === 0){
            this.countHours++;
            if(this.countHours % 5 === 0){
                this.countMinutes = 0;
            }
            rotateHandClock.call(this,this.hours,this.countHours);
            if(this.countHours === 120){
                this.countHours = 0;
            }
        }
    }

    var rotateHandClock = function(id, numRotate){
        id.style = "transform: rotate("+ numRotate * 6 +"deg)";
    }

    var restartClock = function(h,min,sec){
        if(!sec && !min && !h){
            h=0;
            min=0;
            sec=0;
        ...
        var grades = this.countMinutes % 12;
        this.countHours = parseInt((h * 30) + grades);
        rotateHandClock.call(this,this.seconds, sec);
        rotateHandClock.call(this,this.minutes, min);
        rotateHandHourClock.call(this,this.hours, this.countHours);
        grades = this.countMinutes / 12;
        this.countHours = parseInt(h * 5 + grades);
        pauseClock.call(this);
        showDigitaTime.call(this);
    }

    var rotateHandHourClock = function(id, numRotate){
        
        id.style = "transform: rotate("+ numRotate +"deg)";
    }
    ...
    var setTimeClock = function(){
        var time = document.getElementById("inputTime"+this.numCont).value;
        var h = parseInt(time.substring(0,2));
        var min = parseInt(time.substring(3,5));
        var sec = parseInt(time.substring(6,8));
        restartClock.call(this,h,min,sec);
    }

    var handleEvents = function(){
        time = 0;
        var itSelf = this;
        document.getElementById("btnRestart"+itSelf.numCont).addEventListener("click", function(){restartClock.call(itSelf,time,time,time)});
        document.getElementById("btnResume"+itSelf.numCont).addEventListener("click", continueClock.bind(itSelf));
        document.getElementById("btnPause"+itSelf.numCont).addEventListener("click", pauseClock.bind(itSelf));
        document.getElementById("btnSetTime"+itSelf.numCont).addEventListener("click", setTimeClock.bind(itSelf));
    }

    var showDigitaTime = function(){
        document.getElementById("timer"+this.numCont).value = Math.floor(this.countHours/5) + ":" + this.countMinutes + ":" + this.countSeconds;
    }
